...
async function assertRequestedRoleWiring(): Promise<void> {
  const [beatsSource, chatSource, ownEngineSource] = await Promise.all([
    readFile(resolve(process.cwd(), "apps/api/src/routes/beats.ts"), "utf8"),
    readFile(resolve(process.cwd(), "apps/api/src/services/chat-tools.ts"), "utf8"),
    readFile(
      resolve(process.cwd(), "apps/worker/src/processors/own-engine.ts"),
      "utf8",
    ),
  ]);
  ...
  assert.match(
    ownEngineSource,
    /missingExactRequestedMaterialRoles\(picks, requestedRoles\)/,
  );
...
async function assertZapPromptSafety(): Promise<void> {
  ...
  const zapSource = await readFile(
    resolve(process.cwd(), "apps/api/src/routes/zap.ts"),
    "utf8",
  );
  assert.doesNotMatch(
    zapSource,
  ...
  const learnUi = await readFile(
    resolve(process.cwd(), "apps/web/components/LearnMySound.tsx"),
    "utf8",
  );
