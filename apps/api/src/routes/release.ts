...
async function assignUpc(workspaceId: string): Promise<string | null> {
  ...
  const body = (
    prefix + String(sequence).padStart(11 - prefix.length, "0")
  ).slice(0, 11);
  const sum = body
    .split("")
    .reduce(
...
export default async function release(app: FastifyInstance) {
  ...
  app.patch<{ Params: { projectId: string; songId: string } }>(
    ...
    async (req, reply) => {
      ...
      const isrc =
        input.isrc ??
        song.isrc ??
        (splitsValid ? await assignIsrc(workspaceId) : null);
      const upc =
        input.upc ??
        song.upc ??
        (splitsValid ? await assignUpc(workspaceId) : null);
  ...
  app.post<{ Params: { projectId: string; songId: string } }>(
    ...
    async (req, reply) => {
      ...
      const existingLive = await prisma.release.findUnique({
        where: { songId: certification.song.id },
      });
      if (existingLive?.status === "live") {
        return {
          status: "live",
          provider: existingLive.distributor,
          externalId: existingLive.externalId,
          channels: existingLive.channels,
      ...
      const result = await distributeRelease({
        ...
        idempotencyKey:
          "release:" + releaseExport.id + ":" + releaseExport.contentHash,
      });
      ...
      await prisma.$transaction(async tx => {
        ...
        await tx.release.upsert({
          ...
          create: {
            ...
            channels: result.channels as never,
          },
          update: {
            submittedAt,
            distributor: result.provider,
