...
const classifyReferenceSchema = z.object({
  rightsBasis: z.enum(['user-attested', 'facts-only']),
});
...
export default async function taste(app: FastifyInstance) {
  ...
  app.get('/data-lake', async (req) => {
    ...
    const [refTotal, unclassifiedN, failedN, lyricCraftN, trendN, zapN, factsN, generatedN, refs, materials, counts] = await Promise.all([
      ...
      prisma.soundReference.findMany({
        ...
        orderBy: { createdAt: 'desc' },
        take: 500,
        select: { id: true, genre: true, sourceUrl: true, title: true, summary: true, createdAt: true, recipe: true, analysisState: true, rightsBasis: true },
      }),
    ...
    const kind = (r: { sourceUrl: string; recipe: unknown; rightsBasis: string; analysisState: string }) =>
      r.rightsBasis === 'unknown' ? 'unclassified'
      : r.analysisState === 'failed' ? 'failed'
      : r.sourceUrl.startsWith('lyric:') ? 'lyricCraft'
      : r.sourceUrl.startsWith('trend:') ? 'trendSnapshots'
      : r.sourceUrl.startsWith('zap:') ? 'zapped'
      : r.sourceUrl.startsWith('facts:') ? 'referenceFacts'
      : ((r.recipe ?? {}) as { source?: string }).source === 'generated' ? 'selfTraining'
      : 'heardSongs';
    const byKind = {
      heardSongs: Math.max(0, refTotal - unclassifiedN - failedN - lyricCraftN - trendN - zapN - factsN - generatedN),
    ...
    return {
      soundReferences: {
        ...
        byKind,
        genresByKind,
        latest: refs.slice(0, 40).map((r: { id: string; title: string | null; genre: string | null; sourceUrl: string; recipe: unknown; summary: string | null; createdAt: Date; analysisState: string; rightsBasis: string }) => ({
          id: r.id,
          title: r.title,
          genre: r.genre,
          kind: kind(r),
      ...
      orchestration: {
        ...
        referenceFacts: 'measured tempo/key/groove/arrangement feed lane profiles; no prose or expression enters writers',
        unclassified: 'audit-only until the artist confirms ownership or reduces the row to facts-only',
        failed: 'visible for diagnosis, excluded from every generation and lane profile',
  ...
  app.get('/utilization', async (req) => {
    ...
    const [refs, usageRows, materials] = await Promise.all([
      prisma.soundReference.findMany({
        ...
        take: 200,
        select: {
          id: true, title: true, genre: true, sourceUrl: true, recipe: true,
          analysisState: true, rightsBasis: true, active: true, createdAt: true,
        },
      }),
      prisma.referenceUsage.groupBy({
        by: ['referenceId'],
        where: { workspaceId },
        _count: { _all: true },
        _max: { createdAt: true },
    ...
    // Durable ledger → Map<refId, {count, lastAt}>. No capped scan of mutable
    // provider-job JSON and no disappearing history after the 300th render.
    const usage = new Map<string, { count: number; lastAt: Date }>();
    for (const row of usageRows) {
      if (row._max.createdAt) usage.set(row.referenceId, { count: row._count._all, lastAt: row._max.createdAt });
    }
    const harvestedUrls = new Set<string>();
    for (const m of materials) {
      const meta = (m.meta ?? {}) as { sourceUrl?: string; fromSourceUrl?: string };
      const u = meta.fromSourceUrl ?? meta.sourceUrl;
    ...
    return {
      rows: refs.map((r: {
        id: string; title: string | null; genre: string | null; sourceUrl: string;
        recipe: unknown; analysisState: string; rightsBasis: string; active: boolean; createdAt: Date;
      }) => {
        ...
        const origin = r.sourceUrl.startsWith('zap:') || rec.source === 'zap'
          ? ('zap' as const)
          : referenceOrigin(r.sourceUrl, rec, r.rightsBasis);
        ...
        const backfillable =
          !r.sourceUrl.startsWith('zap:') && !r.sourceUrl.startsWith('facts:') &&
          !rec.audioMissing && !rec.deepMeasureExhausted;
        ...
        return {
          ...
          rightsBasis: r.rightsBasis,
          active: r.active,
          deepMeasured: rec.deepMeasured === true,
          usedInRenders: u?.count ?? 0,
          lastUsedAt: u?.lastAt ?? null,
          harvested,
          needsBackfill: backfillable && (!measured || !rec.deepMeasured),
          genreMismatch: detected ? { detected, filed: r.genre } : null,
          learnedAt: r.createdAt,
        };
      }),
      window: {
        ledgeredUses: usageRows.reduce(
          (sum: number, row: { _count: { _all: number } }) => sum + row._count._all,
          0,
        ),
        materialRows: materials.length,
      },
      note: 'usedInRenders is the durable all-time reference-usage ledger. harvested "?" means no exact source-to-stem link was recorded.',
    };
  });
  ...
  app.patch<{ Params: { refId: string } }>('/references/:refId/classification', async (req, reply) => {
    ...
    const reference = await prisma.soundReference.findFirst({
      where: { id: req.params.refId, workspaceId },
      select: { id: true, sourceUrl: true, recipe: true, rightsBasis: true },
    });
    if (!reference) return reply.code(404).send({ error: 'reference_not_found' });

    const recipe = (reference.recipe ?? {}) as Record<string, unknown> & {
      measured?: { engineOk?: boolean };
      metrics?: unknown;
    };
    if (rightsBasis === 'facts-only') {
      const sourceRef = reference.sourceUrl.replace(/^facts:/, '');
      const cleanupJobId = await prisma.$transaction(async (tx) => {
        await tx.soundReference.update({
          where: { id: reference.id },
          data: {
            active: true,
            rightsBasis,
            analysisState: recipe.measured?.engineOk ? 'measured' : 'failed',
            sourceUrl: `facts:${sourceRef}`,
            summary: null,
            recipe: {
              source: 'facts',
              factsOnly: true,
        ...
        return queueAssetDeletion(tx, {
          workspaceId,
          refs: [sourceRef],
          reason: `reference-facts-only:${reference.id}`,
        });
      });
      if (cleanupJobId) {
        void app.dispatchPendingJobs().catch((error) => req.log.error({ err: error }, 'reference cleanup dispatch failed'));
      }
    } else {
      if (reference.rightsBasis !== 'unknown') {
        return reply.code(409).send({ error: 'reference_already_classified' });
      }
      if (/^(zap|trend|facts):/.test(reference.sourceUrl)) {
        return reply.code(400).send({ error: 'facts_reference_cannot_be_reclassified_as_owned' });
      }
      await prisma.soundReference.update({
        ...
        data: {
          active: true,
          rightsBasis,
          analysisState: recipe.measured?.engineOk ? 'measured' : 'inferred',
        },
      });
    }
    return { classified: true, rightsBasis };
  });
  ...
  app.delete<{ Params: { refId: string } }>('/references/:refId', async (req, reply) => {
    ...
    return { deleted: true, retainedUsageEvidence: true };
  });
