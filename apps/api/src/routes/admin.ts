...
export default async function admin(app: FastifyInstance) {
  ...
  const runSchema = z.object({ task: z.enum(['nightly-compound', 'material-harvest', 'measure-backfill', 'learn-backfill', 'listen-back', 'refile-references', 'mine-lexicon', 'lexicon-research', 'wiktionary-harvest', 'wiktionary-burst', 'lexicon-gloss', 'lexicon-verify', 'recert-sweep']) });
