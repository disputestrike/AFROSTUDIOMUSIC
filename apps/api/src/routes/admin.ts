...
export default async function admin(app: FastifyInstance) {
  ...
  const runSchema = z.object({ task: z.enum(['nightly-compound', 'material-harvest', 'rebuild-song-materials', 'repair-legacy-voice-lineage', 'purge-seeded-materials', 'reset-data-lake', 'restore-all-songs', 'consolidate-to-operator', 'measure-backfill', 'learn-backfill', 'listen-back', 'refile-references', 'mine-lexicon', 'lexicon-research', 'wiktionary-harvest', 'wiktionary-burst', 'lexicon-gloss', 'lexicon-verify', 'recert-sweep']) });
