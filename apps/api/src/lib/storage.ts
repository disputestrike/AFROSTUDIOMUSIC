...
const legacyPublicBase = (process.env.S3_PUBLIC_BASE_URL ?? process.env.R2_PUBLIC_URL)?.replace(/\/+$/, '');
...
export function canonicalAssetRef(value: string): string | null {
  ...
  if (location) return location.bucket === bucket ? storageUri(bucket, location.key) : null;
...
export async function presignAssetRef(value: string, expiresInSec = 3600): Promise<string> {
...
export function assertWorkspaceAsset(workspaceId: string, value: string): boolean {
  const canonical = canonicalAssetRef(value);
  if (!canonical) {
    if (value.startsWith('s3://')) {
      throw Object.assign(new Error('invalid_asset_reference'), { statusCode: 400 });
    }
    return false;
  }
  const location = parseStorageUri(canonical)!;
...
export type AudioFormat = 'wav' | 'mp3' | 'flac' | 'ogg' | 'webm' | 'm4a' | 'aiff';
...
export function sniffAudioFormat(bytes: Uint8Array): AudioFormat | null {
  ...
  if ((text(0, 4) === 'RIFF' || text(0, 4) === 'RF64') && text(8, 12) === 'WAVE') return 'wav';
  ...
  if (text(0, 4) === 'FORM' && ['AIFF', 'AIFC'].includes(text(8, 12))) return 'aiff';
  return null;
}
...
export async function verifyUploadedAudio(
  ...
  let head;
  try {
    head = await client().send(new HeadObjectCommand({ Bucket: bucket, Key: ownedKey }));
  } catch {
    throw Object.assign(new Error('uploaded_audio_not_found'), { statusCode: 404 });
  }
  const sizeBytes = Number(head.ContentLength ?? 0);
  if (sizeBytes < 1_000 || sizeBytes > 250 * 1024 * 1024) {
    throw Object.assign(new Error('uploaded_audio_size_invalid'), { statusCode: 413 });
  }
  if (!String(head.ContentType ?? '').toLowerCase().startsWith('audio/')) {
    throw Object.assign(new Error('uploaded_object_is_not_audio'), { statusCode: 415 });
  }
  const object = await client().send(new GetObjectCommand({ Bucket: bucket, Key: ownedKey, Range: 'bytes=0-63' }));
  if (!object.Body) throw Object.assign(new Error('uploaded_audio_unreadable'), { statusCode: 422 });
  const prefix = 'transformToByteArray' in object.Body
  ...
  if (!format) throw Object.assign(new Error('unsupported_or_invalid_audio'), { statusCode: 415 });
  const normalizedExpected = expectedFormat === 'mp4' ? 'm4a' : expectedFormat;
  if (normalizedExpected && normalizedExpected !== format) {
    throw Object.assign(new Error(`audio_format_mismatch_${normalizedExpected}_${format}`), { statusCode: 415 });
  }
