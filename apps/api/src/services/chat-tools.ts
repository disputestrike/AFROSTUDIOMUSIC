...
export function voiceVocalTag(
  voice?: "auto" | "female" | "male" | "duet" | "group" | null
): string | null {
...
export function languageVocalTag(languages?: string[] | null): string {
  ...
  const names =
    list.map(l => LANG_NAMES[normLang(l)] ?? l).join(" + ") || "English";
...
export async function runChatTool(
  args: Ctx & { name: string; args: Record<string, unknown> }
) {
  ...
  switch (name) {
    ...
    case "generate_hooks":
      return generateHooks(
        ctx,
        Number(a.count ?? 3),
        a.languages as string[] | undefined,
        a.refineFrom as string[] | undefined,
        selectionsOf(a),
        a.genre ? String(a.genre) : undefined
      );
    case "score_hooks":
    ...
    case "master_song":
      return masterSongTool(
        ctx,
        String(a.songId),
        a.preset ? String(a.preset) : undefined
      );
    case "make_snippet":
      return makeSnippetTool(
        ctx,
        a.songId ? String(a.songId) : undefined,
        Number(a.startS ?? 0)
      );
    case "reject_hook":
    ...
    case "separate_stems":
      return separateStemsTool(
        ctx,
        a.songId ? String(a.songId) : undefined,
        a.mode === "full" ? "full" : "instrumental"
      );
    case "learn_lyrics":
      return learnLyricsTool(
        ctx,
        String(a.lyrics ?? ""),
        a.genreHint ? String(a.genreHint) : undefined
      );
    case "show_data_lake":
...
async function researchTrendsTool(
  ctx: Ctx,
  a: { genre?: string; region?: string; query?: string }
) {
  ...
  if (!trends) {
    return {
      ...
      hint: "All trend sources failed (YouTube/Apple charts/Tavily/Brave/news) — likely a network issue; try again.",
    };
  }
  ...
  return {
    digest: trends.digest,
    sources: trends.sources,
    chartSource: trends.source,
  };
}
...
async function learnLyricsTool(ctx: Ctx, lyrics: string, genreHint?: string) {
  if (lyrics.trim().length < 40)
    return {
      error: "lyrics_too_short",
      hint: "Paste the full lyric (at least a verse + hook) so there is real craft to study.",
    };
  ...
  if (!existing) {
    charge = await ctx.app.chargeCredits({
      workspaceId: ctx.workspaceId,
      key: "brief_polish",
      refTable: "Workspace",
      refId: ctx.workspaceId,
      idempotencyKey: toolKey(ctx, "learn-lyrics"),
    });
    if (!charge.ok) return { error: "insufficient_credits", ...charge };
  }
  let learned;
  try {
    learned = await learnLyricCraft({
      workspaceId: ctx.workspaceId,
      raw: lyrics,
      genreHint,
    });
  ...
  const inLibrary = await prisma.soundReference.count({
    where: {
      workspaceId: ctx.workspaceId,
...
async function polishBrief(ctx: Ctx, rawIdea: string) {
  ...
  const charge = await ctx.app.chargeCredits({
    ...
    refTable: "Project",
    refId: ctx.projectId,
    idempotencyKey: toolKey(ctx, "polish-brief"),
  });
  if (!charge.ok) return { error: "insufficient_credits", ...charge };
  try {
...
function selectionsOf(a: Record<string, unknown>): Selections | undefined {
  const sel: Selections = {
    ...
    fusionGenres: Array.isArray(a.fusionGenres)
      ? (a.fusionGenres as string[])
      : undefined,
...
async function recentStoriesTold(
  workspaceId: string,
  excludeSongId?: string | null
): Promise<string[]> {
  const drafts: Array<{ title: string | null; body: string }> =
    await prisma.lyricDraft
      .findMany({
        where: {
          project: { workspaceId },
          ...(excludeSongId ? { NOT: { songId: excludeSongId } } : {}),
        },
...
async function generateHooks(
  ...
  if (genre)
    await prisma.project
      .update({ where: { id: ctx.projectId }, data: { genre } })
      .catch(() => {});
  ...
  try {
    const tasteMemory = await memoryContext({
      ...
      query: JSON.stringify({
        genre: project.genre,
        languages: languages ?? project.artist.languages,
    ...
    const storiesTold = refine.length
      ? []
      : await recentStoriesTold(ctx.workspaceId);
    const trendData = await researchTrends({ genre: project.genre }).catch(
      () => null
    );
    ...
    const presong = await presongIntelligence(
      ctx.workspaceId,
      project.genre,
      hmood
    );
    const soundDna = fuseSoundDna({
      laneTargets: hookLane.laneTargets,
      extra: [hardConstraints(project.genre, languages), presong]
        .filter(Boolean)
        .join("\n\n"),
      ...
      learnedRef: await learnedReferenceBrief(ctx.workspaceId, project.genre),
      learnedCraft: await learnedLyricCraftBrief(
        ctx.workspaceId,
        project.genre
      ),
      hitCraft: prompts.hitCraftBrief("hook", hmood),
    });
    ...
    const result = await generateJson<{
      ...
      user: prompts.hookUserPrompt({
        ...
        trends,
        soundDna,
        refineFrom: refine.length ? refine : undefined,
        selections,
    ...
    return {
      ...
      hooks: created.map(
        (c: {
          ...
          const m =
            (c.meta as { viralScore?: number; tiktokMoment?: string } | null) ??
            null;
          return {
            id: c.id,
            text: c.text,
            score: c.score,
            viralScore: m?.viralScore ?? null,
            tiktokMoment: m?.tiktokMoment ?? null,
          };
        }
      ),
...
async function scoreHooks(ctx: Ctx, hookIds: string[]) {
  ...
  try {
    const scores = await scoreItems({
      ...
      items: hooks.map((h: { id: string; text: string }) => ({
        id: h.id,
        text: h.text,
        kind: "hook",
      })),
...
async function generateLyrics(
  ...
  if (genre && genre !== hook.project.genre) {
    await prisma.project
      .update({ where: { id: hook.projectId }, data: { genre } })
      .catch(() => {});
  }
  ...
  const charge = await ctx.app.chargeCredits({
    ...
    idempotencyKey: toolKey(ctx, "generate-lyrics"),
  });
  if (!charge.ok) return { error: "insufficient_credits", ...charge };

  try {
    const lmood = (hook.project.briefs[0] as { mood?: string } | undefined)
      ?.mood;
    ...
    const lpresong = await presongIntelligence(
      ctx.workspaceId,
      laneGenre,
      lmood
    );
    const lyricSoundDna = fuseSoundDna(
      {
        extra: [hardConstraints(laneGenre, languages), lpresong]
          .filter(Boolean)
          .join("\n\n"),
        freshness: await freshnessBrief(ctx.workspaceId),
        palette: await lexiconPalette({
    ...
    type LyricOut = {
      ...
      hookCell?: string;
      anchors?: string[];
      sectionPurposes?: Record<string, string>;
    };
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      if (out?.body && process.env.WRITER_TWO_PASS !== "0") {
        const polished = await generateJson<{
          ...
          user: prompts.lyricPolishPrompt({
            draftTitle: out.title,
            draftBody: out.body,
            genre: laneGenre,
            mood: lmood,
            languages: languages?.length
        ...
        if (polished?.body && polished.body.length > 200) {
          ...
          out = {
            ...
            anchors:
              Array.isArray(polished.anchors) && polished.anchors.length
                ? polished.anchors
                : out.anchors,
      ...
      if (out && typeof out.body === "string" && out.body.trim().length >= 20) {
        firstOutput = out;
        break;
      }
    ...
    if (body.length < 20) {
      throw new Error(
        "lyric came back empty/truncated — regenerating recommended"
      );
    }
    ...
    const craftJson = {
      premise:
        typeof output.premise === "string" && output.premise.trim()
          ? output.premise.trim()
          : null,
      hookCell:
        typeof output.hookCell === "string" && output.hookCell.trim()
          ? output.hookCell.trim()
          : null,
      ...
      sectionPurposes:
        ...
        typeof output.sectionPurposes === "object" &&
        !Array.isArray(output.sectionPurposes)
          ? output.sectionPurposes
          : null,
    };
    ...
    const lyricData = {
      ...
      body,
      cleanVersion:
        typeof output.cleanVersion === "string"
          ? output.cleanVersion
          : undefined,
    ...
    const catRows = await prisma.song.findMany({
      where: {
        ...
        quarantined: false,
        lyric: { isNot: null },
        ...(hook.songId ? { NOT: { id: hook.songId } } : {}),
      },
      select: { id: true, title: true, lyric: { select: { body: true } } },
      take: 300,
      orderBy: { createdAt: "desc" },
    });
    const catalogue = catRows.map(
      (s: { id: string; title: string; lyric: { body: string } | null }) => ({
        id: s.id,
        title: s.title,
        bodyNorm: normalizeLyricBody(s.lyric?.body ?? ""),
      })
    );
    let qa = lyricQaCheck({
    ...
    for (let fix = 0; !qa.ok && fix < 2; fix++) {
      ...
      if (typeof rewrite.hookCell === "string" && rewrite.hookCell.trim())
        craftJson.hookCell = rewrite.hookCell.trim();
      lyricData.body = body;
      lyricData.title = pickLawfulTitle(
        [typeof rewrite.title === "string" ? rewrite.title.trim() : ""],
        craftJson.hookCell || hook.text
      );
      curLangMix =
        (rewrite.languageMix as Record<string, number>) ?? curLangMix;
      lyricData.languageMix = (rewrite.languageMix ??
        lyricData.languageMix) as never;
      qa = lyricQaCheck({
        title: lyricData.title,
        body,
        hookCell: craftJson.hookCell,
        languageMix: curLangMix,
        artistAuthored: false,
        catalogue,
      });
    }
      ...
      if (hook.songId)
        await prisma.song
          .update({
            ...
            data: { quarantined: true, quarantineReason: qa.blocks.join("; ") },
          })
          .catch(() => {});
      await ctx.app.refundCredits({
        workspaceId: ctx.workspaceId,
    ...
    return {
      lyric: { id: lyric.id, title: lyric.title },
      ...(langViolation.length
        ? {
            languageWarning: `still contains: ${langViolation.join(", ")} — regenerate or edit`,
          }
        : {}),
    };
  } catch (error) {
...
async function createBeatJob(
  ...
  a: {
    ...
    withStems?: boolean;
    withVocals?: boolean;
    songEngine?: "suno" | "eleven" | "ace_step" | "minimax" | "own";
    influence?: string;
    languages?: string[];
  ...
  if (a.genre)
    await prisma.project
      .update({ where: { id: ctx.projectId }, data: { genre: a.genre } })
      .catch(() => {});
  ...
  if (a.withVocals) {
    ...
    artistAuthored = !!(lyric as { artistAuthored?: boolean } | null)
      ?.artistAuthored;
    lyrics = artistAuthored
      ? (lyric?.body ?? undefined)
      : (lyric?.cleanVersion ?? lyric?.body ?? undefined);
    ...
    if (!lyrics)
      return {
        error: "no_lyrics — write the lyrics first, then make the full song",
      };
  }
  ...
  const dna = a.fusionGenres?.length
    ? laneDna(a.genre, { mood: a.mood, fusionGenres: a.fusionGenres })
    : laneDna(a.genre, { mood: a.mood });
  const learned = await learnedReferenceBrief(
    ctx.workspaceId,
    a.genre,
    a.pinnedReferenceId
  );
  const learnedTags = await learnedStyleTags(
    ctx.workspaceId,
    a.genre,
    a.pinnedReferenceId
  );
  ...
  const measuredTags = await learnedMeasuredTags(
    ctx.workspaceId,
    a.genre,
    a.pinnedReferenceId
  );
  const trainingUsage = await learnedUsage(
    ctx.workspaceId,
    a.genre,
    a.pinnedReferenceId
  );
  ...
  const blueprint = a.pinnedReferenceId
    ? await blueprintForReference(ctx.workspaceId, a.pinnedReferenceId)
    : null;
  ...
  const dnaTags = [
    ...
    ...(blueprint
      ? [
          `structure ${blueprint.sections.length} sections`,
          ...(blueprint.bpm ? [`${blueprint.bpm} bpm exact`] : []),
        ]
      : []),
  ];
  ...
  if (a.withVocals && lyrics) {
    if (artistAuthored) {
      ...
      const langs = a.languages?.length
        ? a.languages
  ...
  const finalDnaTags = [
    ...
    ...[voiceVocalTag(a.voice), languageVocalTag(a.languages)].filter(
      (t): t is string => !!t
    ),
    // VOCAL-RHYTHM DIRECTIVE: the sung text carries the syllables; this tag
    // carries the pocket (parity with the REST path).
    ...((sungForm as { applied?: boolean } | null)?.applied
      ? [
          "vocal delivery: syncopated Afro phrasing, off-beat pushes into the hook, melisma runs held on open vowels",
        ]
      : []),
    ...dnaTags,
    ...
    payload: jobId => ({
      ...
      input: {
        ...
        durationS:
          ...
          (a.withVocals ? genreSignature(a.genre).durationS : 60),
  ...
  return {
    ...
    status: "queued",
    mode: a.withVocals ? "full_song_with_vocals" : "instrumental",
  };
}
...
async function generateCoverArt(
  ...
  const charge = await ctx.app.chargeCredits({
    workspaceId: ctx.workspaceId,
    key:
      (a.quality ?? "medium") === "high" ? "cover_art_high" : "cover_art_low",
    refTable: "Project",
    refId: ctx.projectId,
    idempotencyKey,
  });
...
async function generateStoryboard(
  ...
  return {
    concept: { id: concept.id, title: concept.title, shots: storyboard },
  };
}

async function renderVideo(
  ctx: Ctx,
  a: { conceptId: string; shotIndex?: number }
) {
  if (!ctx.projectId) return { error: "no_project_in_thread" };
  ...
  const usage = videoRenderUsage(shots, a.shotIndex);
  if (!usage) return { error: "invalid_video_shot_selection" };
...
async function runDropTool(
  ...
  a: {
    ...
    bpm?: number;
    withVocals?: boolean;
    songEngine?: "suno" | "eleven" | "ace_step" | "minimax";
    languages?: string[];
    mood?: string;
    fusionGenres?: string[];
    influence?: string;
  ...
  await polishBrief(
    {
      ...ctx,
      operationKey: ctx.operationKey ? `${ctx.operationKey}:brief` : undefined,
    },
    a.theme
  );
  const drops: Array<{
  ...
  for (let i = 0; i < count; i++) {
    ...
    const hk = (await generateHooks(
      ...
      undefined,
      a.genre
    )) as { hooks?: Array<{ id: string; text: string; score: number | null }> };
    let hooks = hk?.hooks ?? [];
    if (!hooks.length) continue;
    if (hooks.every(h => h.score == null)) {
      const sc = (await scoreHooks(
        stepCtx,
        hooks.map(h => h.id)
    ...
    const beat = (await createBeatJob(stepCtx, {
      genre,
      bpm,
      withVocals: a.withVocals ?? true,
      songEngine: a.songEngine,
      languages: a.languages,
      mood: a.mood,
      fusionGenres: a.fusionGenres,
    ...
    if (a.songTitle && producedSongId) {
      ...
      await prisma.song
        .update({ where: { id: producedSongId }, data: { title: t } })
        .catch(() => undefined);
      await prisma.lyricDraft
        .updateMany({ where: { songId: producedSongId }, data: { title: t } })
        .catch(() => undefined);
    }
    drops.push({
...
async function masterSongTool(ctx: Ctx, songId: string, preset?: string) {
  ...
  // Master only canonical certified audio, matching the REST path.
  ...
  if (legacySource) {
    ...
    const masterMeta = currentMaster?.meta as
      ...
      | null;
    const sourceMixId =
      typeof masterMeta?.sourceMixId === "string" ? masterMeta.sourceMixId : null;
    const sourceMixHash =
      typeof masterMeta?.sourceContentHash === "string"
        ? masterMeta.sourceContentHash
        : null;
    const sourceMix =
      sourceMixId && sourceMixHash
    ...
    const beatId =
      typeof sourceLineage?.beatId === "string" ? sourceLineage.beatId : null;
    const beatContentHash =
      typeof sourceLineage?.beatContentHash === "string"
        ? sourceLineage.beatContentHash
        : null;
    const vocalRenderIds =
      Array.isArray(sourceLineage?.vocalRenderIds) &&
      sourceLineage.vocalRenderIds.every(
        (id): id is string => typeof id === "string" && id.length > 0
      )
        ? [...new Set(sourceLineage.vocalRenderIds)]
        : null;
    const [sourceBeat, certifiedVocalCount] =
      sourceMix && beatId && beatContentHash && vocalRenderIds
        ? await Promise.all([
    ...
    if (
      ...
      !sourceBeat ||
      !vocalRenderIds ||
      certifiedVocalCount !== vocalRenderIds.length
    ) {
      return {
    ...
    const candidateMeta = candidate?.meta as
      | { source?: unknown; sourceContentHash?: unknown }
      | null;
  ...
  const finished =
    current.type === "master" ||
    (current.type === "beat" &&
      current.id === latestBeat?.id &&
      ["minimax", "suno"].includes(latestBeat?.provider ?? "")) ||
...
async function makeSnippetTool(
  ctx: Ctx,
...
async function rejectHookTool(ctx: Ctx, hookId: string) {
  ...
  return {
    ok: true,
    hookId,
    note: "Rejected — the taste engine will avoid this pattern.",
  };
}
...
async function listBeatsTool(ctx: Ctx) {
  ...
  const beats = await prisma.beatAsset.findMany({
    ...
    orderBy: { createdAt: "desc" },
    take: 20,
    include: { stems: { select: { role: true } } },
  });
  return {
    beats: beats.map(
      (b: {
        ...
        stems: b.stems.map((s: { role: string }) => s.role),
        url: b.url,
      })
    ),
  };
  ...
  return {
    ...
    songs: songs.map(
      (s: {
        ...
        audioUrl:
          s.masters[0]?.url ?? s.mixes[0]?.url ?? s.beats[0]?.url ?? null,
      })
    ),
  };
}
...
async function predictHitTool(ctx: Ctx, songId: string | undefined) {
  ...
  const prediction = await arReadSong(
    ctx.app,
    ctx.workspaceId,
    song.id,
    toolKey(ctx, "predict-hit")
  );
  if (!prediction)
    return {
...
async function separateStemsTool(
  ctx: Ctx,
  songId: string | undefined,
  ...
  return {
    ...
    note:
      mode === "instrumental"
        ? "Making the true instrumental: the finished song with the voice taken out and everything else kept, loudness-matched to the original. It lands in the song download in a few minutes."
        : "Stems (vocals/drums/bass/other) will appear in the song download shortly.",
  };
}

/** Forge isolated loops (the material layer's raw stock) for a genre. */
async function forgeMaterialsTool(
  ctx: Ctx,
  a: { genre: string; bpm?: number; keySignature?: string }
) {
  ...
  return {
    forging: jobs,
    keySignature,
    note: `Forging ${jobs.length} isolated ${a.genre} loops at ${bpm}bpm in ${keySignature}. QC-passed loops land in the material library; then call assemble_beat.`,
  };
}

/** Assemble the EXACT beat from real material — Claude arranges, worker places. */
async function assembleBeatTool(
  ctx: Ctx,
  a: { genre: string; bpm?: number; keySignature?: string; vibe?: string }
) {
  if (!ctx.projectId) return { error: "no_project_in_thread" };
  const bpm = Number(a.bpm ?? 108);
...
async function makeMaterialBeatTool(
  ctx: Ctx,
  a: { genre?: string; bpm?: number; vibe?: string; songId?: string }
) {
  if (!ctx.projectId) return { error: "no_project_in_thread" };
  const project = await prisma.project.findFirstOrThrow({
    where: { id: ctx.projectId, workspaceId: ctx.workspaceId },
...
async function setReleaseRightsTool(
  ctx: Ctx,
  a: {
    songId: string;
  ...
  if (!a.splitSheet?.length) {
    return {
      error:
        a.nativeReviewOk == null
          ? "nothing_to_set"
          : "named_native_review_required",
      message:
        "Native-language approval requires a named human attestation on the Release screen.",
    };
  }
  await prisma.song.update({
    where: { id: song.id },
  ...
  const sum = a.splitSheet.reduce(
    (total, split) => total + (Number(split.share) || 0),
    0
  );
  return {
    ok: true,
