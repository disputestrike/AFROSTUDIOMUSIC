...
/**
 * FLAGSHIP BRIDGE — first-party tool: generate the song in the operator's own
 * flagship-studio account (top-tier audio, clean rights), then bring the file
 * back to master + score. §1.11 THE WALL + ADDENDUM R-1: this component ships
 * ZERO vendor strings — the brand name, open URL, and tips arrive at runtime
 * from the ADMIN-GATED bridge-export endpoint, so nothing vendor-identifying
 * ever lands in the public JS bundle. Rendered only behind the admin unlock,
 * and loaded as a lazy chunk only when that unlock exists.
 */
...
interface Export {
  ...
  hasLyrics: boolean;
  tips: string;
  brandName: string; // arrives from the admin-gated API — never bundled
  openUrl: string;
}
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-black/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="mt-8 w-full max-w-lg rounded-2xl border border-white/10 bg-[#0b0b12] p-5 shadow-card" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-center justify-between">
          <h2 className="font-display text-xl">Flagship bridge</h2>
        ...
        {!data && !err && <div className="flex items-center gap-2 py-8 text-sm text-slate-400"><Loader2 className="h-4 w-4 animate-spin" /> Preparing your bridge pack…</div>}

        {data && (
          <div className="space-y-4">
            <p className="text-xs text-slate-400">
              Generate <span className="text-slate-200">“{data.title}”</span> in your own {brand} account — best audio, and the rights stay yours. Paste these two fields, then bring the file back to master + score it.
            </p>
            ...
            {/* Lyrics */}
            {data.hasLyrics ? (
              <Field label="Lyrics" value={data.lyrics} multiline copied={copied === 'lyrics'} onCopy={() => copy(data.lyrics, 'lyrics')} />
            ) : (
              <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 p-2.5 text-xs text-amber-200">No lyrics on this song yet — write them first, then take it across the bridge.</div>
            )}

            <a href={data.openUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 rounded-full bg-brand-gradient px-4 py-2 text-sm font-medium text-ink shadow-glow">
              Open {brand} <ExternalLink className="h-3.5 w-3.5" />
            </a>
            ...
            <div className="border-t border-white/10 pt-4">
              <div className="mb-2 text-sm font-medium text-slate-200">Bring it back</div>
              {phase === 'done' ? (
