...
export function ReleaseReadiness({
  ...
  const load = useCallback(async () => {
    setLoadError("");
    const path = songId
      ? "/projects/" + projectId + "/release/" + encodeURIComponent(songId)
      : "/projects/" + projectId + "/release";
    const next = await api.get<Status>(path);
    setStatus(next);
    setSplits(
  ...
  }, [api, projectId, songId]);

  useEffect(() => {
    void load().catch(error => setLoadError((error as Error).message));
  }, [load]);
  ...
  async function waitForJob(jobId: string): Promise<void> {
    ...
    throw new Error(
      "The job is still running. Its result will remain available in Jobs."
    );
  }
  ...
  async function acceptSplits() {
    ...
    await runAction("splits", async () => {
      ...
      setStatus(next);
      setMessage(
        "Split-sheet accepted and sealed to this exact 100% allocation."
      );
    });
  }
  ...
  async function runRightsScan() {
    ...
    await runAction("rights", async () => {
      ...
      await load();
      setMessage(
        "Rights scan completed against the current certified artifacts."
      );
    });
  }
          ...
          <div className="flex items-start gap-3 text-amber-200">
            ...
            <div className="min-w-0">
              ...
              <button
                ...
                className="mt-3 inline-flex min-h-9 items-center gap-2 rounded border border-slate-700 px-3 py-2 text-sm text-slate-200 hover:border-slate-500"
              >
    ...
    return (
      <section className="mt-8 flex min-h-40 items-center justify-center border-t border-white/10">
        <Loader2
          ...
          aria-label="Loading release status"
        />
      </section>
    );
  }
    ...
    return (
      <section className="mt-8 border-t border-white/10 pt-6">
        ...
        <p className="mt-2 text-sm text-slate-500">
          A rendered song will appear here for certification.
        </p>
      </section>
    );
  }
    ...
    <section className="mt-8 border-t border-white/10 pt-6">
      ...
      {status.distribution && (
        <p className="mt-1 text-xs text-slate-500">
          Distribution: {status.distribution.status} via{" "}
          ...
          <ul className="mt-3 space-y-2">
            {greenLight?.checks.map(check => (
              ...
              >
                ...
                <div className="min-w-0">
                  <div
                    className={check.ok ? "text-slate-200" : "text-slate-400"}
                  >
                    {check.name}
                  </div>
                  {check.detail && (
          ...
          {cover ? (
            <div className="mt-3 grid grid-cols-[96px_1fr] gap-3">
              ...
              <div className="min-w-0 text-xs text-slate-400">
                ...
                <div className="mt-1">
                  {cover.approved ? "Approved" : "Approval required"}
                </div>
                {!cover.approved && (
                  <button
                    type="button"
            ...
            <p className="mt-3 text-sm text-slate-500">
              No cover has been generated for this project.
            </p>
          )}
        </div>
      </div>
      ...
      <div className="mt-4 rounded-lg border border-white/10 bg-slate-950/40 p-4">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <span
            className={
              "font-mono text-sm " +
              (totalValid ? "text-emerald-400" : "text-amber-400")
            }
          >
        ...
        <div className="mt-3 space-y-2">
          {splits.map((split, index) => (
            <div
              key={index}
              className="grid grid-cols-[minmax(0,1fr)_minmax(100px,140px)_72px_36px] gap-2"
            >
              <input
                value={split.name}
                onChange={event =>
              ...
              <input
                ...
                onChange={event =>
                  setSplits(current =>
                    current.map((row, rowIndex) =>
                      rowIndex === index
                        ? { ...row, share: Number(event.target.value) }
                        : row
                    )
                  )
                }
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            disabled={
              busy !== null ||
              !totalValid ||
              splits.some(split => !split.name.trim())
            }
            ...
            <button
              ...
              disabled={
                ...
                requiredLanguages.some(
                  language => !reviewLanguages.includes(language)
                )
              }
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-lg border border-white/10 bg-slate-950/40 p-4">
          ...
          <p className="mt-2 text-xs text-slate-500">
            Verified WAV, MP3, cover, metadata, lyrics, splits, provenance,
