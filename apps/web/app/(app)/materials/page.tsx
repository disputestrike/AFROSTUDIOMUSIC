...
import { materialCanAutoAssemble, materialCoverage } from "@afrohit/shared";
...
const ORIGIN_LABEL: Record<string, string> = {
  forged: "forged",
  synth: "synth",
  artist_stem: "your stems",
  provider_stem: "provider stems",
};
...
export default function MaterialsPage() {
  ...
  useEffect(() => {
    ...
    void load().then(mats => {
      ...
      for (const m of mats)
        if (m.genre) byGenre.set(m.genre, [...(byGenre.get(m.genre) ?? []), m]);
      const best = [...byGenre.entries()].sort(
  ...
  async function pollJob(
    ...
    throw new Error(
      "timed out — it may still land on the shelf in a few minutes"
    );
  }
  ...
  async function forgeKit() {
    ...
    try {
      ...
      if (!total) {
        setForging("");
        setForgeNote(
          res.note ?? "Kit already stocked — assembling from the shelf."
        );
        void load();
        return;
      }
      ...
      const failed: string[] = [];
      setForging(
        `Forging ${total} loops${res.keySignature ? ` in ${res.keySignature}` : ""} — they land one at a time (rate-limit spacing)…`
      );
      await Promise.allSettled(
        res.forging.map(async f => {
          try {
          ...
          setForging(
            `${ok + failed.length}/${total} finished (${ok} on the shelf${failed.length ? `, ${failed.length} failed` : ""})…`
          );
          void load();
        })
      );
      ...
      if (failed.length)
        setForgeErr(
          `${ok}/${total} loops landed. Failed: ${failed.join(" · ")}`
        );
      void load();
    } catch (e) {
      setForging("");
      setForgeErr(
        `${(e as Error).message.slice(0, 160)} — any loops already queued may still arrive; the shelf refreshes below.`
      );
      void load();
    }
  }
  ...
  function friendlyAssembleError(msg: string): string {
    if (/not_enough_material/.test(msg))
      return `Not enough ${genre.replace(/_/g, " ")} loops near ${bpm}bpm on the shelf — forge a kit at this bpm first.`;
    if (/insufficient_credits/.test(msg))
      return "Daily generation cap reached — try again tomorrow or raise the cap in Settings.";
  ...
  async function assemble() {
    ...
    try {
      ...
      }>("/materials/assemble", { projectId, genre, bpm });
  ...
  const usable = (materials ?? []).filter(
    m =>
      m.genre === genre &&
      m.readiness === "ready" &&
      m.qualityState === "passed" &&
      !!m.rightsBasis &&
      m.rightsBasis !== "unknown" &&
      materialCanAutoAssemble(m) &&
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-10">
      ...
      <p className="mt-2 max-w-2xl text-sm text-slate-400">
        Workspace loops from your uploads, studio synthesis, licensed material,
        or connected generation providers.
        <span className="text-slate-200">
          {" "}
          The worker verifies each selected file and records exactly where it
          was used.
        </span>
      </p>
      ...
      {integrity && integrity.totalLoops > 0 && (
        <p className="mt-2 text-xs text-slate-500">
          <span className="text-slate-300">
            {integrity.totalLoops} loop{integrity.totalLoops === 1 ? "" : "s"}
          </span>
          {" · "}
          {integrity.distinctFiles} distinct file
          {integrity.distinctFiles === 1 ? "" : "s"}
          {" · "}
          <span
            className={
              integrity.duplicates > 0 ? "text-amber-400" : "text-emerald-400"
            }
          >
            {integrity.duplicates} duplicate
            {integrity.duplicates === 1 ? "" : "s"}
          </span>
          {" · "}
          {integrity.usedMaterials} used in {integrity.totalUses} assembly
          receipt{integrity.totalUses === 1 ? "" : "s"}
          {" — "}
          {Object.entries(integrity.byOrigin)
            .map(([o, n]) => `${ORIGIN_LABEL[o] ?? o} ${n}`)
            .join(" / ")}
        </p>
      )}
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-2xl glass p-4">
          <div className="flex items-center gap-2 font-grotesk text-sm font-medium text-slate-200">
            <Hammer className="h-4 w-4 text-afrobrand-400" /> Forge a kit
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <input
              ...
              min={60}
              max={180}
              onChange={e => setBpm(Number(e.target.value) || 108)}
              className="w-24 rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-slate-200"
            />
            <span className="text-xs text-slate-500">bpm</span>
            <button
              onClick={() => void forgeKit()}
              disabled={!!forging}
              className="flex items-center gap-2 rounded-full bg-brand-gradient px-4 py-2 text-sm font-medium text-ink shadow-glow disabled:opacity-50"
            >
              {forging ? (
          ...
          {forging && (
            <div className="mt-2 text-xs text-afrobrand-300">{forging}</div>
          )}
          {forgeNote && (
            <div className="mt-2 rounded-lg border border-emerald-500/30 bg-emerald-500/10 p-2.5 text-xs text-emerald-300">
              {forgeNote}
            </div>
          )}
          {forgeErr && (
          ...
          <p className="mt-2 text-[11px] text-slate-500">
            One isolated loop per role (~$0.10 each). Loops arrive ~30s apart —
            the provider rate-limits, we pace it.
          </p>
        </div>

        <div className="rounded-2xl glass p-4">
          <div className="flex items-center gap-2 font-grotesk text-sm font-medium text-slate-200">
            <Layers className="h-4 w-4 text-afrobrand-400" /> Assemble the exact
            beat
          </div>
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <select
              ...
              className="max-w-[220px] rounded-lg border border-white/10 bg-black/30 px-3 py-2 text-sm text-slate-200"
            >
              {projects.map(p => (
            ...
            <button
              ...
              disabled={!!assembling || !projectId || !canAssemble}
              className="flex items-center gap-2 rounded-full bg-brand-gradient px-4 py-2 text-sm font-medium text-ink shadow-glow disabled:opacity-50"
            >
              {assembling ? (
                <Loader2 className="h-4 w-4 animate-spin" />
          ...
          <div className="mt-1.5 text-[11px] text-slate-500">
            ...
            {canAssemble ? (
              <span className="text-emerald-400">
                {usableRoles.size} roles ready at {bpm}bpm:{" "}
                {[...usableRoles].join(", ")}
              </span>
            ) : (
              <span className="text-amber-400">
                needs 5 beds with 2 rhythm, 1 low-end, and 1 tonal role near{" "}
                {bpm}bpm (now {coverage.beds}/{coverage.rhythm}/
                {coverage.lowEnd}/{coverage.tonal})
              </span>
            )}
          </div>
          {assembling && (
            <div className="mt-2 text-xs text-afrobrand-300">{assembling}</div>
          )}
          {assembleErr && (
            <div className="mt-2 rounded-lg border border-red-500/30 bg-red-500/10 p-2.5 text-xs text-red-300">
              {assembleErr}
            </div>
          )}
          ...
          {beatUrl && (
            <div className="mt-3">
              <div className="text-xs text-emerald-400">
                Your exact beat — assembled from the shelf:
              </div>
              <audio
                controls
                preload="none"
      ...
      {materials === null && !loadErr && (
        <div className="mt-8 text-sm text-slate-500">Loading the shelf…</div>
      )}
      {materials !== null && materials.length === 0 && (
        <div className="mt-8 rounded-2xl glass p-6 text-sm text-slate-400">
          The shelf is empty — forge your first kit above.
        </div>
      )}
      {[...shelf.entries()].map(([g, items]) => (
        <section key={g} className="mt-8">
          <h2 className="font-grotesk text-lg font-medium text-slate-200">
            {g.replace(/_/g, " ")}{" "}
            <span className="text-xs text-slate-500">
              ({items.length} loop{items.length === 1 ? "" : "s"})
            </span>
          </h2>
              ...
              >
                <div className="flex items-center justify-between text-sm text-slate-200">
                  <span className="flex items-center gap-1.5">
                    ...
                    {(m.variant ?? 0) >= 2 && (
                      <span className="rounded bg-white/10 px-1 py-px text-[9px] font-medium text-afrobrand-300">
                        v{m.variant}
                      </span>
                    )}
                  </span>
                  ...
                  <span className="text-[10px] uppercase tracking-wide text-slate-500">
                    {(() => {
                      const o =
                        m.origin ??
                        (m.source === "artist_stem"
                          ? "artist_stem"
                          : m.source === "provider_stem"
                            ? "provider_stem"
                            : "forged");
                      return o === "artist_stem"
                        ? "your stem"
                        : o === "provider_stem"
                          ? "provider stem"
                          : o === "synth"
                ...
                <div className="mt-1 text-xs text-slate-500">
                  {m.bpm ? `${m.bpm}bpm` : ""}
                  {m.keySignature ? ` · ${m.keySignature}` : ""}
                  {m.bars ? ` · ${m.bars} bars` : ""}
                </div>
                <div className="mt-1 flex flex-wrap gap-1 text-[10px]">
                  <span
                    className={
                      m.readiness === "ready" && materialCanAutoAssemble(m)
                        ? "text-emerald-300"
                        : m.readiness === "rejected"
                          ? "text-red-300"
                          : "text-amber-300"
                    }
                  >
                  ...
                  <span
                    className={
                      m.usageCount > 0 ? "text-emerald-300" : "text-slate-600"
                    }
                  >
