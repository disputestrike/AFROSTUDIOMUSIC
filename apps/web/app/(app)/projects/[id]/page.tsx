...
export default async function ProjectPage({
  ...
  searchParams: Promise<{ produce?: string; song?: string }>;
}) {
  const { id } = await params;
  const { produce, song } = await searchParams;
  ...
  return (
    <div className="mx-auto max-w-6xl px-6 py-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <div className="mt-1 text-sm text-slate-400">
            {p.artist.stageName} · {p.genre} ·{" "}
            {p.bpm ? `${p.bpm} bpm` : "no BPM"}
          </div>
        </div>
        <Link
      ...
      <ReleaseReadiness projectId={p.id} songId={song} />
          ...
          {p.beats.length ? (
            p.beats.map(b => (
              <AudioRow
                key={b.id}
                url={b.url}
                label={`Beat · ${b.bpm ?? "?"} bpm`}
              />
            ))
          ) : (
            <Empty hint="Approve lyrics, then create a beat." />
          )}
        </Section>
        <Section title="Vocals" small>
          {p.vocalRenders.length ? (
            p.vocalRenders.map(v => (
              <AudioRow key={v.id} url={v.url} label={v.role} />
            ))
          ) : (
            <Empty hint="Need a Voice Profile and approved lyrics." />
          )}
        </Section>
        <Section title="Mixes" small>
          {p.mixes.length ? (
            p.mixes.map(m => (
              <AudioRow key={m.id} url={m.url} label={`Mix · ${m.preset}`} />
            ))
          ) : (
            <Empty hint="Create a mix once vocals are approved." />
          )}
        </Section>
        <Section title="Masters" small>
          {p.masters.length ? (
            p.masters.map(m => (
              <AudioRow key={m.id} url={m.url} label={`Master · ${m.preset}`} />
            ))
          ) : (
            <Empty />
          )}
...
function Section({
  ...
  return (
    <section className="mt-8">
      <h2 className={small ? "font-display text-xl" : "font-display text-2xl"}>
        {title}
      </h2>
