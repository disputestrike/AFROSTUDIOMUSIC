...
import { parseStorageUri, storageUri } from '../../../packages/shared/src/storage-uri';
import { escapeHtml } from '../../../packages/shared/src/html';
import { redactSensitiveText } from '../../../packages/shared/src/redact';
import { createShareLinkSchema, voiceConsentInputSchema } from '../../../packages/shared/src/schemas';
import { VOICE_CONSENT_TEXT, VOICE_CONSENT_VERSION } from '../../../packages/shared/src/voice-consent';
...
async function main() {
  ...
  assert.equal((await assertSafeUrl('https://youtube.com/watch?v=test')).ok, false);
  ...
  const redacted = redactSensitiveText('Bearer topsecret https://cdn.example/file.wav?X-Amz-Signature=abc user@example.com');
  assert.doesNotMatch(redacted, /topsecret|X-Amz-Signature|user@example/);
  assert.equal(createShareLinkSchema.safeParse({ songId: 'cm12345678901234567890123', targetUrl: 'javascript:alert(1)' }).success, false);
  assert.equal(createShareLinkSchema.safeParse({ songId: 'cm12345678901234567890123', targetUrl: 'https://music.example/song' }).success, true);
  const consent = {
  ...
  assert.equal(voiceConsentInputSchema.safeParse({ ...consent, consentText: 'I agree to something else entirely.' }).success, false);

  const capture = JSON.stringify({ workspaceId: 'workspace-1234567890', pack: 'pack_25', creditsCents: 250_000 });
  assert.deepEqual(validateCreditPackCapture(capture, { value: '25.00', currency_code: 'USD' }), {
    workspaceId: 'workspace-1234567890',
    pack: 'pack_25',
    creditsCents: 250_000,
  });
  assert.equal(validateCreditPackCapture(capture, { value: '1.00', currency_code: 'USD' }), null);
  assert.equal(validateCreditPackCapture(capture, { value: '25.00', currency_code: 'EUR' }), null);
  ...
  try {
    ...
    const session = signSession({ sub: 'user-1', workspaceId: 'workspace-1', role: 'OWNER' });
    ...
    if (oldAdminSecret === undefined) delete process.env.ADMIN_SECRET; else process.env.ADMIN_SECRET = oldAdminSecret;
    if (oldWebUrl === undefined) delete process.env.WEB_URL; else process.env.WEB_URL = oldWebUrl;
