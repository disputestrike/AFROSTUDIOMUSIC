...
async function runOfflineTests(): Promise<void> {
  ...
  const TS: Array<[string, string]> = [
    ...
    ["Auto-clip (hook-first vertical shorts off the master)", "test-clips.ts"],
    ["Auto-visuals (lyric video + visualizer + thumbnails off the master)", "test-visuals.ts"],
    ...
    ["AfroOne reliable sing (forge once/reuse + guaranteed singing)", "test-afroone-reliable-sing.ts"],
    ...
    ["Real training reaches production (Apache base + approve→promote)", "test-real-training.ts"],
    ...
    ["Engine-agnostic conditioning (genre/deAfro/swing/CD-lyrics/influence/mood/lang/tone → SELECTED engine, not own)", "test-engine-agnostic.ts"],
