...
const workers = [
  ...
  // AUTO-VISUALS (Phase 3) — the lyric video + audio-reactive visualizer + 3-5
  // thumbnails, generated by ffmpeg/image EDIT off the master audio + lyrics +
  // cover (no re-render, $0), the moment a SONG finishes (no video needed). Its
  // own dedicated lane, off the render lanes AND off the concurrency-1 lake lane
  // so visuals land promptly. The processor is fail-soft (never throws), so this
  // job always completes.
  makeWorker("visuals", async (job: { data: never; name: string }) => {
    if (job.name !== "generate-visuals")
