...
function certifiedTimestamp(value: unknown, field: string): string {
  ...
  if (!Number.isFinite(timestamp)) throw new Error('release_lineage_invalid_' + field);
  return new Date(timestamp).toISOString();
}

function parseDerivation(metadata: JsonRecord | null): CertifiedReleaseLineageBase['derivation'] {
  const raw = record(metadata?.derivedFrom);
  if (!raw) return null;
  if (raw.type !== 'beat' && raw.type !== 'mix' && raw.type !== 'master' && raw.type !== 'external') {
  ...
  const parentBeatId = claim.kind === 'derived_mix'
  ...
  const parentVocalRenderContentHashes = parentBeatId
    ? strictHashes(claim.vocalRenderContentHashes, 'derivation_parent_vocal_hashes')
    : [];
...
export function parseReleaseMixLineage(meta: unknown): ParsedReleaseMixLineage {
  ...
  if (direct) {
    ...
    const recordedAt = certifiedTimestamp(direct.recordedAt, 'direct_upload_recorded_at');
    const certifiedAt = certifiedTimestamp(direct.certifiedAt, 'direct_upload_certified_at');
    ...
    const parentSourceContentHash = direct.sourceKind === 'owned_derivative'
      ? certifiedHash(direct.parentSourceContentHash, 'direct_upload_parent_source_hash')
      : null;
    const parentClaimHash = direct.sourceKind === 'owned_derivative'
      ? certifiedHash(direct.parentClaimHash, 'direct_upload_parent_claim_hash')
      : null;
    return {
      kind: 'direct_owned_upload',
      sourceKind: direct.sourceKind,
      sourceContentHash: certifiedHash(
  ...
  if (lineage.originKind === 'direct_owned_upload') {
    return {
      ...
      source: {
        ...
        rightsConfirmationVersion: lineage.directUpload.rightsConfirmationVersion,
...
export async function resolveCertifiedReleaseLineage(options: {
  ...
  if (options.audio.kind === 'master') {
    ...
    if (
      ...
      || certifiedHash(masterMeta?.sourceContentHash, 'master_source_mix_hash') !== expectedMixHash
    ) {
  ...
  const lineageBeatIds = [...new Set([
    source.beatId,
    ...(source.derivation?.parentBeatId ? [source.derivation.parentBeatId] : []),
  ])];
  const allVocalIds = [...new Set([
    ...source.vocalRenderIds,
    ...(source.derivation?.parentVocalRenderIds ?? []),
  ])].sort();
  const [beat, inheritedBeat, vocals, materialUsages, referenceUsages] = await Promise.all([
    ...
    prisma.materialUsage.findMany({
      where: { workspaceId: options.workspaceId, songId: options.songId, beatId: { in: lineageBeatIds } },
      orderBy: { id: 'asc' },
      select: {
        id: true,
        providerJobId: true,
  ...
  source.derivation?.parentVocalRenderIds.forEach((id, index) => {
    ...
    if (existing && existing !== hash) throw new Error('release_lineage_vocal_hash_conflict:' + id);
...
export async function processRights(payload: RightsPayload): Promise<void> {
  ...
  try {
    ...
    const beatIds = lineage.beat
      ? [...new Set([
          lineage.beat.id,
          ...(lineage.derivation?.parentBeatId ? [lineage.derivation.parentBeatId] : []),
        ])]
      : [];
    const [hook, approvals, materialUsages, referenceUsages] = await Promise.all([
      ...
      beatIds.length ? prisma.materialUsage.findMany({
        where: {
          workspaceId: payload.workspaceId,
          songId: payload.songId,
          beatId: { in: beatIds },
      ...
      beatIds.length ? prisma.referenceUsage.findMany({
        where: {
          workspaceId: payload.workspaceId,
          songId: payload.songId,
          beatId: { in: beatIds },
    ...
    const providers = [
      beat
        ...
        : {
            kind: 'music',
            provider: lineage.directUpload.sourceKind === 'workspace_upload'
              ? 'artist_upload'
    ...
    const provenance = lineage.originKind === 'direct_owned_upload'
      ? {
          source: {
            kind: lineage.originKind,
      ...
      : {
          source: { kind: lineage.originKind, derivedFrom: lineage.derivation },
          beat,
          vocals: lineage.vocals,
    ...
    const aiDisclosure = lineage.originKind === 'direct_owned_upload'
      ? {
          distroDisclosure: 'Artist-supplied finished recording; production origin not inferred by AfroHit',
          credits: {
    ...
    const receipt = await prisma.rightsReceipt.create({
      data: {
        ...
        humanContribution: payload.audioRightsAttestation?.note
    ...
    await prisma.$transaction(async (tx) => {
      ...
      await tx.song.update({ where: { id: payload.songId }, data: { releaseReady: false } });
