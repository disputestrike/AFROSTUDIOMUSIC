...
const MAX_ANALYZE_BYTES = 256 * 1024 * 1024;
const ANALYZE_DOWNLOAD_TIMEOUT_MS = 2 * 60_000;
...
export async function processAnalyze(p: AnalyzePayload) {
  ...
  let temporaryAnalyzeDir: string | null = null;
  let sourcePurgeDelegated = false;
  try {
    if (!p.factsOnly && p.rightsBasis !== "user-attested") {
      throw new Error(
        "full reference learning requires an explicit user-attested rights basis"
      );
    }
    // Re-fetch through the worker's redirect-aware SSRF guard and enforce hard
    // byte/time ceilings. Native codecs only receive this local copy.
    const sourceBytes = await downloadToBuffer(p.url, {
      maxBytes: MAX_ANALYZE_BYTES,
      timeoutMs: ANALYZE_DOWNLOAD_TIMEOUT_MS,
    });
    if (sourceBytes.length < 1_000)
      throw new Error("analysis source is empty or too small");
    const sourceContentHash = createHash("sha256")
      .update(sourceBytes)
      .digest("hex");
    temporaryAnalyzeDir = await mkdtemp(join(tmpdir(), "afrohit-analyze-"));
    const localSourcePath = join(temporaryAnalyzeDir, "source-audio");
    await writeFile(localSourcePath, sourceBytes);
    ...
    const project = await prisma.project.findFirst({
      where: { id: p.projectId, workspaceId: p.workspaceId },
      select: { artistId: true, genre: true },
    });
    if (!project)
      throw new Error("analysis project missing or outside workspace");

    // FACTS-ONLY: measure the numbers, learn no expression, keep no audio.
    if (p.factsOnly) {
      let metrics: Awaited<ReturnType<typeof measureAudioQuality>> | null =
        null;
      try {
        metrics = await measureAudioQuality(localSourcePath);
      } catch {
        metrics = null;
      }
      let measured = unknownAnalysis("engine-unavailable");
      try {
        if (await dspAvailable()) measured = await measureAudio(sourceBytes);
      } catch (err) {
        console.warn(
          "[analyze] facts-only DSP measure failed:",
          (err as Error)?.message
        );
      }
      const sourceUrl = `facts:sha256:${sourceContentHash}`;
      ...
      const prior = (existing?.recipe ?? {}) as {
        measured?: MeasuredAnalysis;
        metrics?: unknown;
      };
      const preserveMeasured =
        existing?.analysisState === "measured" &&
        prior.measured?.engineOk === true &&
        !measured.engineOk;
      const effectiveMeasured = preserveMeasured ? prior.measured! : measured;
      const effectiveMetrics = preserveMeasured
        ? (prior.metrics ?? metrics)
        : metrics;
      const coverage = analysisCoverage(effectiveMeasured);
      const referenceId =
        existing?.id ??
        `facts_${createHash("sha256").update(`${p.workspaceId}|${p.url}`).digest("hex").slice(0, 24)}`;
      const reference = await prisma.soundReference
        ...
        .catch((err: unknown) => {
          console.warn(
            "[analyze] facts reference write failed:",
            (err as Error)?.message
          );
          return null;
        });
    ...
    try {
      if (await ffmpegAvailable()) {
        const clip = await extractClip(sourceBytes, 12, 60);
        if (clip.length > 2000) {
          analyzeUrl = await uploadBytes({
            workspaceId: p.workspaceId,
            kind: "reference",
            bytes: clip,
    ...
    const profile = await analyzeAudio(
      await resolveAssetForProvider(analyzeUrl),
      replicateApiKey,
      {
    ...
    try {
      if (await dspAvailable()) {
        ...
        measured = await measureAudio(sourceBytes);
      }
    } catch (err) {
    ...
    console.log(
      `[analyze] ear: engineOk=${measured.engineOk} measured=${coverage.measured}/${coverage.total} tempo=${JSON.stringify(measured.tempoBpm?.value)}`
    );
    ...
    const learnedGenre =
      normalizeGenre(profile.genre) ?? project?.genre ?? null;
    ...
    const reference = await (
      sourceContentHash
        ...
        : prisma.soundReference.create({
            data: { workspaceId: p.workspaceId, ...referenceData },
          })
    )
      // A failed write here silently loses a LEARNED reference — log it.
      .catch((err: unknown) => {
        console.warn(
          "[analyze] SoundReference write failed:",
          (err as Error)?.message
        );
        return null;
      });
    ...
    if (reference?.id && process.env.DSP_STEMS !== "0" && measured.engineOk) {
      try {
        ...
        console.warn(
          "[analyze] deep-measure enqueue failed:",
          (e as Error)?.message
        );
      }
    }
    ...
    await prisma.analyticsEvent
      ...
      .catch((err: unknown) =>
        console.warn(
          "[analyze] taste event write failed:",
          (err as Error)?.message
        )
      );
    ...
    if (temporaryAnalyzeDir) {
      await rm(temporaryAnalyzeDir, { recursive: true, force: true }).catch(
        () => {}
      );
    }
  }
