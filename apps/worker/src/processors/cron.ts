...
import { debitCredits, refundDebitedCredits } from "../lib/credits";
import {
  jobDoneEmail,
  morningDropEmail,
  releaseRadarEmail,
...
export async function processMorningDrop() {
  ...
  return runWithBrainContext(
    { forceTier: "bulk", runId: "morning-drop" },
    morningDropRun
  );
}
async function morningDropRun() {
  if (!(await isAutonomyEnabled("morning_drop"))) {
    console.log("[morning-drop] disabled by operator (autonomy off) — skipped");
    return;
  }
  const artists = await prisma.artist.findMany({
    where: {
      morningDrop: true,
      workspace: { suspendedAt: null, deletedAt: null },
    },
  ...
  for (const artist of artists) {
    try {
      ...
      if (!charge.ok) {
        console.log(
          `[morning-drop] skip ${artist.stageName} — insufficient credits`
        );
        continue;
      }
      ...
      const queryEmbedding = memoryRows.some(row =>
      ...
      const scores = await scoreItems({
        artist: artist as never,
        items: created.map((h: { id: string; text: string }) => ({
          id: h.id,
          text: h.text,
          kind: "hook" as const,
        })),
      ...
      const top = created
        .map((h: { id: string; text: string }) => ({
          text: h.text,
          score: scores.find(s => s.id === h.id)?.overall ?? null,
        }))
        .sort(
      ...
      console.log(
        `[morning-drop] ${artist.stageName}: ${created.length} hooks, email=${emailStatus}`
      );
    } catch (err) {
...
export async function processReleaseRadar() {
  ...
  const byWorkspace = new Map<
    string,
    Array<{ country: string | null; events: number }>
  >();
  for (const r of rows) {
  ...
  for (const [workspaceId, countries] of byWorkspace) {
    try {
      ...
      if (!delivery.ok) {
        console.warn(
          `[release-radar] email not sent for workspace ${workspaceId}: ${delivery.error}`
        );
      }
    } catch (err) {
      console.error(
        `[release-radar] failed for workspace ${workspaceId}:`,
        err
      );
    }
  }
...
function radarSlice(): string[] {
  const runs = Math.max(
    1,
    Math.min(12, parseInt(process.env.ZAP_RUNS_PER_DAY ?? "4", 10) || 4)
  );
  const pool = [...GENRES] as string[];
  const chunk = 5;
  const slot = Math.floor(
    new Date().getUTCHours() / Math.max(1, Math.floor(24 / runs))
  );
...
export async function processZapRadar() {
  ...
  return runWithBrainContext(
    { forceTier: "bulk", runId: "zap-radar" },
    zapRadarRun
  );
}
async function zapRadarRun() {
  if (!(await isAutonomyEnabled("zap_radar"))) {
    console.log("[zap-radar] disabled by operator (autonomy off) — skipped");
    return;
  }
  ...
  for (const ws of workspaces) {
    ...
    try {
      for (const genre of radarSlice()) {
        ...
        if (
          !trends ||
          (trends.source !== "apple_charts" && trends.source !== "youtube")
        )
          continue;
        ...
        for (const song of songs) {
          if (learned >= RADAR_MAX_PER_RUN) break;
          const marker = `zap:${`${song.artist ?? ""}-${song.title}`
            .toLowerCase()
            .replace(/[^a-z0-9]+/g, "-")
            .slice(0, 80)}`;
          const exists = await prisma.soundReference.findFirst({
            where: { workspaceId: ws.id, sourceUrl: marker },
            select: { id: true },
          });
          ...
          if (!charge.ok) {
            learned = RADAR_MAX_PER_RUN;
            break;
          }
          try {
            const craft = await extractSongCraft({
              title: song.title,
              artist: song.artist,
              genre,
            });
            if (!craft?.craft?.length)
              throw new Error("zap radar returned no usable craft");
            ...
            console.warn(
              `[zap-radar] ${marker} was not learned; charge reversed:`,
      ...
      console.log(
        `[zap-radar] ws ${ws.id}: learned ${learned} new trending song(s) into the lake`
      );
    } catch (err) {
...
export async function notifyJobDone(jobId: string) {
  try {
    ...
    const url =
      ...
      ((out?.bundle as Record<string, unknown> | undefined)?.mp3 as
        | string
        | undefined) ??
      null;
    ...
    if (!delivery.ok) {
      console.warn(
        `[notify] email not sent for job ${jobId}: ${delivery.error}`
      );
    }
  } catch (err) {
