...
assert.throws(
  ...
  /release_lineage_source_claim_invalid/,
  'component claims must explicitly bind both vocal arrays, including an empty set'
);
assert.throws(
  () =>
    parseReleaseMixLineage({
...
const directParentClaimHash = releaseEvidenceHash(directParentClaim);
const ownedDerivative = parseReleaseMixLineage({
  directOwnedUpload: {
    schemaVersion: 1,
    sourceKind: 'owned_derivative',
    rightsConfirmation: { version: 1, confirmed: true },
    sourceContentHash: hash('8'),
    parentSourceContentHash: hash('9'),
  ...
  derivedFrom: {
    ...
    claim: directParentClaim,
  },
});
assert.equal(ownedDerivative.sourceKind, 'owned_derivative');
assert.equal(ownedDerivative.parentClaimHash, directParentClaimHash);
assert.throws(
  () =>
    parseReleaseMixLineage({
      directOwnedUpload: {
        ...
        rightsConfirmation: { version: 1, confirmed: true },
        sourceContentHash: hash('8'),
        parentSourceContentHash: hash('7'),
        parentClaimHash: directParentClaimHash,
        recordedAt: '2026-07-15T12:02:00.000Z',
        certifiedAt: '2026-07-15T12:03:00.000Z',
      },
  ...
  /release_lineage_direct_upload_parent_mismatch/,
  'the direct and derivation branches must identify the same parent bytes'
);

const parityFixtures = [
  {
    name: 'component claim',
    meta: {
      source: {
        beatId: 'beat-1',
        beatContentHash: hash('a'),
  ...
  {
    name: 'missing component arrays',
    meta: { source: { beatId: 'beat-1', beatContentHash: hash('a') } },
  },
  {
    name: 'missing direct certification timestamp',
    meta: {
      directOwnedUpload: {
        schemaVersion: 1,
        sourceKind: 'workspace_upload',
  ...
  {
    name: 'ambiguous source branches',
    meta: {
      source: {
        beatId: 'beat-1',
        beatContentHash: hash('a'),
  ...
  {
    name: 'owned derivative parent mismatch',
    meta: {
      directOwnedUpload: {
        schemaVersion: 1,
        sourceKind: 'owned_derivative',
...
for (const fixture of parityFixtures) {
  const dbAccepts = releaseMixSourceClaim(fixture.meta) !== null;
  let workerAccepts = true;
  try {
    parseReleaseMixLineage(fixture.meta);
  } catch {
    workerAccepts = false;
  }
  assert.equal(
    workerAccepts,
    dbAccepts,
    `DB and worker lineage validators diverged for ${fixture.name}`
  );
}
...
assert.equal(
  'url' in evidence.beat,
  false,
  'private asset URLs must not enter lineage fingerprints'
);
...
const rightsSource = readFileSync(
  new URL('../src/processors/rights.ts', import.meta.url),
  'utf8'
);
const exportSource = readFileSync(
  new URL('../src/processors/export.ts', import.meta.url),
  'utf8'
);
assert.match(
  rightsSource,
  /row\.mixId/,
  'master lineage must follow Master.mixId'
);
assert.match(
  rightsSource,
  /const beatIds = lineage\.beat/,
  'rights usages must be restricted to certified current and inherited beats'
);
assert.match(
  rightsSource,
  /release_lineage_direct_upload_hash_mismatch/,
  'direct uploads must match the certified mix bytes'
);
assert.match(
  rightsSource,
  /release_lineage_master_source_receipt_mismatch/,
  'master metadata must bind its exact source mix'
);
assert.match(
  rightsSource,
  /vocalRenderContentHashes/,
  'component lineage must bind exact vocal hashes'
);
assert.match(
  rightsSource,
  /for \(const artifact of componentArtifacts\.values\(\)\)/,
  'rights certification must byte-verify each source without unbounded parallel downloads'
);
assert.match(
  rightsSource,
  /release_source_material_/,
  'material evidence must be checked against the stored source bytes'
);
assert.match(
  rightsSource,
  /release_lineage_external_parent_not_durable/,
  'external-only parent snapshots must remain unreleasable until stored durably'
);
assert.match(
  exportSource,
  /const backing = lineage\.beat/,
  'export backing must use the exact lineage beat'
);
assert.match(
  exportSource,
  /if \(lineage\.beat\)/,
  'direct uploads must not fabricate a backing track'
);
assert.match(
  exportSource,
  /receiptPayload\.lineage/,
  'export must reject a stale or lineage-free rights receipt'
);
assert.match(
  exportSource,
  /lineage: exactLineage/,
  'export fingerprint and manifest must include exact IDs and hashes'
);
assert.doesNotMatch(
  exportSource,
