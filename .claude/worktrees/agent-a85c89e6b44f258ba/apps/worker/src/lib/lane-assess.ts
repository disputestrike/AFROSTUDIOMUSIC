...
export async function laneGrounding(
  workspaceId: string,
  genre?: string | null
): Promise<LaneGrounding> {
...
export async function loadLaneProfile(
  workspaceId: string,
  genre?: string | null
): Promise<LaneProfile | null> {
  ...
  for (const r of rows) {
    ...
    else if (origin === "owned-upload" || origin === "facts-only")
      nonSelf.push(rec.measured);
  }
  if (nonSelf.length >= 3) {
    const profile = buildLaneProfile(genre, "genre", [...nonSelf, ...self], {
      minRefs: 3,
    });
  ...
  if (priors.length) {
    ...
    if (Object.keys(pp.features).length) {
      console.log(
        `[lane] ${genre}: expert-prior profile in use (${nonSelf.length} external refs; ${self.length} self excluded — self-promotion locked)`
      );
      return pp;
    }
  }
...
export async function assessLaneCompliance(opts: {
  ...
  | { status: "unavailable"; assessedGenre: string | null; reason: string }
  | { status: "failed"; assessedGenre: string | null; reason: string }
> {
  try {
    // LANE_ASSESS default-ON (only off when explicitly '0') per the FINAL INSTRUCTION.
    if (process.env.LANE_ASSESS === "0")
      return {
        status: "unavailable",
        assessedGenre: opts.genre ?? null,
        reason: "disabled",
      };
    if (!opts.genre)
      return {
    ...
    if (!profile)
      return {
        status: "unavailable",
        assessedGenre: opts.genre,
        reason: "lane_profile_unavailable",
      };
    ...
    if (songId) {
      await prisma.song
        ...
        .catch((e: unknown) =>
          console.warn(
            "[lane-assess] song write-through failed:",
            (e as Error)?.message
          )
        );
    }
    console.log(
      `[lane-assess] ${opts.genre}: compliance=${score.overall}/100 drift=${score.drift.severity} repairs=${plan.repairs.length}`
    );
    return {
      status: "measured",
      assessedGenre: opts.genre,
    ...
    return {
      ...
      assessedGenre: opts.genre ?? null,
      reason:
        (err as Error)?.message?.slice(0, 200) || "lane_assessment_failed",
    };
  }
}
