...
| 7 | A trained AfroOne candidate is better and active | Dataset hash, consent snapshot, provider job, bound evaluation, improvement threshold, promotion transaction, rollback pointer, and a frozen evaluation holdout excluded by source ID, lineage family, and byte hash | Nine real rights-cleared holdout recordings and legitimate evaluator receipt |
