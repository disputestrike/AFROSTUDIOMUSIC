...
export const RELEASE_REVIEW_LANGUAGES = [
  'yo',
  'ig',
  'ha',
  'zu',
...
export interface ReleaseCertification {
  song: {
    ...
    project: {
      genre: string;
      artist: { stageName: string; languages: string[]; references: unknown };
    };
  };
  audio: {
    kind: 'master' | 'mix';
...
function hashMatches(
  ...
  return (
    ...
    releaseEvidenceHash(payload) === hash
  );
}
...
export function releaseMixSourceClaim(meta: unknown): JsonRecord | null {
  ...
  let derivationClaim: JsonRecord | null = null;
  if (rawDerivation) {
    const type = rawDerivation.type;
    const id =
      typeof rawDerivation.id === 'string' ? rawDerivation.id.trim() : '';
  ...
  if (direct) {
    ...
    const recordedAt = certifiedTimestamp(direct.recordedAt);
    const certifiedAt = certifiedTimestamp(direct.certifiedAt);
    if (
      ...
      !recordedAt ||
      !certifiedAt ||
      Date.parse(certifiedAt) < Date.parse(recordedAt)
    ) {
      return null;
    }
  ...
  const beatId =
    typeof component?.beatId === 'string' && component.beatId.trim()
      ? component.beatId.trim()
      : null;
  const beatIds =
    component?.beatIds === undefined
      ? []
  ...
  const rawVocalHashes = Array.isArray(component?.vocalRenderContentHashes)
    ...
    : null;
  const vocalRenderContentHashes =
    rawVocalHashes?.map(certifiedContentHash) ?? null;
  if (
...
async function releaseClaimParentIsCurrent(
  db: PrismaClient | Prisma.TransactionClient,
  options: { projectId: string; songId: string },
  sourceClaim: JsonRecord,
  visited = new Set<string>(),
  depth = 0
): Promise<boolean> {
  const derivation = record(sourceClaim.derivedFrom);
  if (!derivation) return true;
  const type = String(derivation.type ?? '');
  const id = typeof derivation.id === 'string' ? derivation.id.trim() : '';
  const sourceContentHash = certifiedContentHash(derivation.sourceContentHash);
  const claimHash = certifiedContentHash(derivation.claimHash);
  if (
    !id ||
    !sourceContentHash ||
    !claimHash ||
  ...
  if (type === 'beat') {
    const parent = await db.beatAsset.findFirst({
      where: {
        id,
    ...
    if (!parent?.contentHash) return false;
    return (
      releaseEvidenceHash({
        schemaVersion: 1,
        kind: 'derived_mix',
        beatId: parent.id,
        beatContentHash: parent.contentHash.toLowerCase(),
        vocalRenderIds: [],
        vocalRenderContentHashes: [],
      }) === claimHash
    );
  ...
  if (type === 'mix') {
    const parent = await db.mix.findFirst({
      where: {
        id,
        projectId: options.projectId,
        songId: options.songId,
    ...
    return (
      !!parentClaim &&
      releaseEvidenceHash(parentClaim) === claimHash &&
      (await releaseClaimParentIsCurrent(
        db,
        options,
        parentClaim,
        nextVisited,
  ...
  const parent = await db.master.findFirst({
    where: {
      id,
      projectId: options.projectId,
      songId: options.songId,
  ...
  const parentMixHash = certifiedContentHash(parent?.mix?.contentHash);
  if (
    !parent?.mix ||
    !parentMixHash ||
    ...
    certifiedContentHash(parentMeta?.sourceContentHash) !== parentMixHash
  ) {
    return false;
  }
...
export async function loadReleaseCertification(
  ...
  const song = await db.song.findFirstOrThrow({
    ...
    include: {
      project: {
        include: {
          artist: {
            select: { stageName: true, languages: true, references: true },
          },
        },
      },
      lyric: true,
  ...
  const hasCoverSelection = Object.prototype.hasOwnProperty.call(
    options,
    'coverAssetId'
  );
  ...
  const releaseLocksAudio =
    !!releaseHead &&
    ['submitting', 'submitted', 'accepted', 'live'].includes(
      releaseHead.status
    );
  const lockedAudio = !releaseLocksAudio
    ...
    : releaseHead.audioAssetId &&
      ...
      : null;
  const selectedAudioRef = Object.prototype.hasOwnProperty.call(
    options,
    'audioAsset'
  )
    ? options.audioAsset
  ...
  const masterPromise =
    selectedAudioRef === null || selectedAudioRef?.kind === 'mix'
      ? Promise.resolve(null)
      : db.master.findFirst({
          where: {
            ...(selectedAudioRef?.kind === 'master'
              ? { id: selectedAudioRef.id }
              : {}),
            songId: song.id,
  ...
  const mixPromise =
    selectedAudioRef === null || selectedAudioRef?.kind === 'master'
      ? Promise.resolve(null)
      : db.mix.findFirst({
          where: {
            ...(selectedAudioRef?.kind === 'mix'
              ? { id: selectedAudioRef.id }
              : {}),
            songId: song.id,
            projectId: song.projectId,
  ...
  const [master, mix, cover, rightsReceipt, attestations] = await Promise.all([
    ...
    db.releaseAttestation.findMany({
      where: {
        songId: song.id,
        kind: { in: ['split_sheet', 'native_language'] },
      },
      orderBy: { createdAt: 'desc' },
    }),
  ]);
  ...
  const masterSourceReceiptMatches =
    !!master?.mix &&
    masterMeta?.sourceMixId === master.mix.id &&
    certifiedContentHash(masterMeta?.sourceContentHash) ===
      master.mix.contentHash?.toLowerCase();
  const certifiedSourceMix =
    master?.mix &&
    ...
    master.mix.verifiedAt
      ? {
          kind: 'mix' as const,
          id: master.mix.id,
          contentHash: master.mix.contentHash,
        }
      : null;
  ...
  const sourceParentCurrent = sourceClaim
    ? await releaseClaimParentIsCurrent(
        db,
        { projectId: song.projectId, songId: song.id },
        sourceClaim
      )
    : false;
  ...
  const artifactSnapshot: ReleaseArtifactSnapshot = {
    audio: canonicalAudio?.contentHash
      ...
      : null,
    cover: cover?.contentHash
      ? { id: cover.id, contentHash: cover.contentHash }
      : null,
  ...
  const receiptHashValid =
    !!rightsReceipt && hashMatches(receiptPayload, rightsReceipt.hash);
  const receiptCurrent =
    receiptHashValid &&
    receiptPayload?.artifactFingerprint === artifactFingerprint;
  const rightsCheck = record(receiptPayload?.rightsCheck);
  const audioRecognition = record(receiptPayload?.audioRecognition);
  const rightsRisk =
    typeof rightsCheck?.overallRisk === 'string'
      ? rightsCheck.overallRisk
      : null;
  const audioRecognitionClear = ['clear', 'matched_cleared'].includes(
    String(audioRecognition?.status ?? '')
  );
  const rightsOk = rightsCheck?.okToExport === true && audioRecognitionClear;

  const splitSheet = normalizeSplitSheet(song.splitSheet);
  const splitAttestation =
    attestations.find(attestation => attestation.kind === 'split_sheet') ??
    null;
  ...
  const nativeAttestation =
    attestations.find(attestation => attestation.kind === 'native_language') ??
    null;
  const nativePayload = record(nativeAttestation?.payload);
  const reviewedLanguages = normalizedLanguages(nativePayload?.languages);
  const nativeAttested =
    !!nativeAttestation &&
    hashMatches(nativePayload, nativeAttestation.hash) &&
    nativePayload?.attested === true &&
    requiredNativeLanguages.every(language =>
      reviewedLanguages.includes(language)
    );

  const shareTotal = splitSheet.reduce(
    (total, entry) => total + entry.share,
    0
  );
  const baseReadiness = evaluateReleaseReadiness({
    ...
    splits: {
      total: shareTotal,
      count: splitSheet.length,
      attested: splitAttested,
    },
    rights: {
  ...
  const lineageCheck: ReleaseReadinessCheck = {
    ...
    ok:
      (!master?.mixId || certifiedSourceMix !== null) &&
    ...
    detail:
      sourceClaimHash === null
        ...
        : !sourceParentCurrent
          ? 'derived audio parent is missing, stale, external-only, or does not match its certified claim'
          : master?.mixId
            ? certifiedSourceMix
              ? 'master is bound to certified source mix ' +
                certifiedSourceMix.id
