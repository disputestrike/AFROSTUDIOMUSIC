/**
 * THE PLAYABLE-ASSET LAW — which audio "the song" currently is, and what its
 * measured arrangement says. Moved verbatim from apps/api/src/lib/
 * current-playable-asset.ts (which now re-exports from here) so the WORKER's
 * auto-assemble trigger resolves a concept's audio through the exact same
 * pure law every API surface uses — one law, two I/O shells, zero drift.
 */
import type { SongBlueprint } from './blueprint';
